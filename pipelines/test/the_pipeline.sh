...
while [ "$step" -lt "4" ]; do
	...
	then
		...
		if [ "$step" == 1]
		then
			sbatch C:\Users\Rylan\Documents\HackKstate\PipelineBuilder\programs/run_add_count.sh --count 50 --input $directoryname/add_count.txt  --done $directoryname/.steps/program2.done
			uuid=program2
		fi
		let "step++"
	fi
